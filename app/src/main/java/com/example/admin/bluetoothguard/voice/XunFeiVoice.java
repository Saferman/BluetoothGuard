...
//isLast等于true 时会话结束。
    ...
    public void Synthesize(String textVoice) {
        ...
        //3.开始合成
        mTts.startSpeaking(textVoice, null);// 最后一个参数可以指定Listener
    }

    // 语音合成
    public void SynthesizeWithListener(String textVoice, SynthesizerListener myListener) {
        //1.创建 SpeechSynthesizer 对象, 第二个参数： 本地合成时传 InitListener
        SpeechSynthesizer mTts= SpeechSynthesizer.createSynthesizer(activity, null);
        //2.合成参数设置，详见《 MSC Reference Manual》 SpeechSynthesizer 类
        //设置发音人（更多在线发音人，用户可参见 附录13.2
        ...
        mTts.setParameter(SpeechConstant.TTS_AUDIO_PATH, "./sdcard/iflytek.pcm");
        //3.开始合成
        mTts.startSpeaking(textVoice, myListener);// 最后一个参数可以指定Listener
    }
